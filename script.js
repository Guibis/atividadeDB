...
const DB = {
    clientes: [
        { cpf: '123.456.789-00', nome: 'João da Silva', telefone: '(11) 91234-5678' },
        { cpf: '987.654.321-00', nome: 'Maria Oliveira', telefone: '(11) 99876-5432' },
    ],
    pets: [
        { id: 1, animal: 'Cachorro', nome: 'Rex', raca: 'Labrador', dono_cpf: '123.456.789-00' },
        { id: 2, animal: 'Gato', nome: 'Mimi', raca: 'Persa', dono_cpf: '987.654.321-00' },
    ],
    produtos: [
        { id: 1, nome: 'Ração Premium 5kg', preco: 89.90, estoque: 30 },
        { id: 2, nome: 'Shampoo Pet 500ml', preco: 24.50, estoque: 15 },
        { id: 3, nome: 'Coleira Ajustável M', preco: 35.00, estoque: 20 },
    ],
    compras: [
        {
            id: 1, cliente_cpf: '123.456.789-00', data: '2025-04-15', valor_total: 114.40,
            produtos: [{ id: 1, nome: 'Ração Premium 5kg', qtd: 1, valor: 89.90 }, { id: 2, nome: 'Shampoo Pet 500ml', qtd: 1, valor: 24.50 }]
        },
        {
            id: 2, cliente_cpf: '987.654.321-00', data: '2025-04-18', valor_total: 35.00,
            produtos: [{ id: 3, nome: 'Coleira Ajustável M', qtd: 1, valor: 35.00 }]
        },
    ]
};
...
const clienteNome = cpf => { const c = DB.clientes.find(c => c.cpf === cpf); return c ? c.nome : cpf; };
...
function closeModal(id) {
    ...
    // reset inputs
    document.getElementById(id).querySelectorAll('input, select, textarea').forEach(el => { el.value = ''; });
}
...
function compraCard(c) {
    const prods = c.produtos.map(p => `<span class="badge badge-orange">${p.nome} x${p.qtd}</span>`).join('');
    return `
...
function renderClientes() {
    ...
    if (!DB.clientes.length) { tbody.innerHTML = ''; empty.style.display = 'block'; return; }
...
function renderPets() {
    ...
    if (!DB.pets.length) { tbody.innerHTML = ''; empty.style.display = 'block'; return; }
    empty.style.display = 'none';
    const emojiAnimal = a => ({ 'Cachorro': '🐶', 'Gato': '🐱', 'Pássaro': '🦜', 'Coelho': '🐰', 'Hamster': '🐹' }[a] || '🐾');
    tbody.innerHTML = DB.pets.map(p => `<tr>
...
function renderProdutos() {
    ...
    if (!DB.produtos.length) { tbody.innerHTML = ''; empty.style.display = 'block'; return; }
    empty.style.display = 'none';
    tbody.innerHTML = DB.produtos.map(p => `<tr>
    <td><span class="badge badge-teal">#${p.id}</span></td>
    <td><strong>${p.nome}</strong></td>
    <td>${fmt(p.preco)}</td>
    <td>
...
function renderCompras() {
    ...
    if (!DB.compras.length) { cont.innerHTML = ''; empty.style.display = 'block'; return; }
    empty.style.display = 'none';
    cont.innerHTML = [...DB.compras].reverse().map(c => compraCard(c)).join('');
}
...
function salvarCliente() {
    ...
    if (!cpf || !nome || !tel) { toast('Preencha todos os campos!', 'error'); return; }
    if (DB.clientes.find(c => c.cpf === cpf)) { toast('CPF já cadastrado!', 'error'); return; }
...
function salvarPet() {
    ...
    if (!animal || !nome || !dono) { toast('Preencha os campos obrigatórios!', 'error'); return; }
...
function salvarProduto() {
    ...
    if (!nome || isNaN(preco) || isNaN(estoque)) { toast('Preencha todos os campos!', 'error'); return; }
...
function deleteCliente(cpf) {
    ...
    renderPage('clientes'); renderDashboard();
    toast('Cliente removido.');
}
function deletePet(id) {
    if (!confirm('Remover este pet?')) return;
    DB.pets = DB.pets.filter(p => p.id !== id);
    renderPage('pets'); renderDashboard();
    toast('Pet removido.');
}
function deleteProduto(id) {
    if (!confirm('Remover este produto?')) return;
    DB.produtos = DB.produtos.filter(p => p.id !== id);
    renderPage('produtos'); renderDashboard();
